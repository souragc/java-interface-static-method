...
`git clone https://github.com/ByteLegendQuest/java-interface-static-method`

A project directory named `java-interface-static-method` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-interface-static-method` directory which is created just now.
